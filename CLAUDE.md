...
- **Automation Ecosystem**: 8 hooks, 34 agents, 29 skills, 13 commands

---

## CLI-First Policy (MANDATORY)

**Codex CLI** and **Gemini CLI** are PRIMARY tools, not optional fallbacks.

### Investigation / Analysis Domain → Codex CLI FIRST
When the task involves: code analysis, bug investigation, security audit, performance profiling,
code review, cross-validation, dependency analysis, or any investigative work:
1. **MUST** invoke `codex` CLI before providing analysis results
2. Use codex for: stack detection, code quality scan, test validation, architectural review
3. Claude provides synthesis and final recommendations after codex output

### Design / UX / Visualization Domain → Gemini CLI FIRST
When the task involves: UI design, UX flows, visual assets, diagram generation, SVG creation,
design profiles, mockups, wireframes, or any visual/design work:
1. **MUST** invoke `gemini` CLI before providing design outputs
2. Use gemini for: design profiles, Mermaid diagrams, SVG generation, CSS suggestions
3. Claude provides integration and refinement after gemini output
...
### Dev Lifecycle Agents (18 + codex-reviewer)
...
| codex-reviewer | **[PRIMARY]** Codex CLI 기반 교차 검증 (코드/테스트/문서) |
...
| error-recovery.sh | PostToolUse | **Hook 실패 자동 복구, 지수적 백오프 재시도** |
