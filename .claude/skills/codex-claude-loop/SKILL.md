...
|-------|-----------|----------|
| `gpt-5.3-codex` | `low` | 빠른 검증, 간단한 리뷰 |
| `gpt-5.3-codex` | `medium` | Plan 검증, 코드 리뷰 |
| `gpt-5.3-codex` | `high` | 심층 코드 리뷰, 아키텍처 분석 |
| `gpt-5.3-codex` | `xhigh` | 대규모 아키텍처 리뷰, 보안 감사 |
...
AskUserQuestion first: model (`gpt-5.3-codex`) + reasoning (`low`/`medium`/`high`/`xhigh`)
