...
| Task | Model | Reasoning | Upgrade Condition | Upgrade Reasoning |
|------|-------|-----------|-------------------|-------------------|
| Phase 1 코드 분석 | `gpt-5.3-codex` | `medium` | 파일 100+개 또는 LoC 50K+ | `high` |
| Phase 1 스택 감지 | `gpt-5.3-codex` | `medium` | monorepo 또는 언어 3+개 | `high` |
| Phase 3.5 스키마 검증 | `gpt-5.3-codex` | `low` | - (항상 경량) | - |
| Phase 3.5 코드 리뷰 | `gpt-5.3-codex` | `xhigh` | - (항상 중량) | - |
| Phase 3.5 교차 참조 | `gpt-5.3-codex` | `low` | - (패턴 매칭만) | - |
...
3. reasoning 결정: 조건 충족 시 업그레이드 reasoning effort 사용
...
### General execution
```bash
echo "PROMPT" | codex exec -m gpt-5.3-codex --sandbox read-only -C TARGET_DIR
```
...
Provide file:line evidence for all claims." | codex exec -m gpt-5.3-codex --sandbox read-only -C {repo_path}
```
...
Provide confidence levels (high/medium/low) for each." | codex exec -m gpt-5.3-codex --sandbox read-only -C {repo_path}
```
...
Report issues as: {severity: error|warning|info, file, line, message}" | codex exec -m gpt-5.3-codex --sandbox read-only -C workspace/{project}/
```
